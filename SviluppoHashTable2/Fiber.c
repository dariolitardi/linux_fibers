...
*/
	/*
			...
			for (c=0; c<FLS_SIZE; c++){
				if(lista_fiber_iter->bitmap_fls[c]==0){
					index=c;
					break;
				}
			}
			...
			lista_fiber_iter->bitmap_fls[index]=1;	
...
static bool flsFree(long index){
	...
	*/
     ...
     if(lista_fiber_iter->bitmap_fls[index]== 0){
     ...
     lista_fiber_iter->bitmap_fls[index]=0;
...
static long long flsGetValue(long pos){
	...
	*/
	...
	if(lista_fiber_iter->bitmap_fls[pos] == 0){
