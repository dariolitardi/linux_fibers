...
int Pre_Handler_Readdir(struct kprobe *p, struct pt_regs *regs){ 
	...
	if(strcmp(file->f_path.dentry->d_name.name,"fibers")==0){
		...
		hash_for_each_possible_rcu(processi, fiber_processo, node, id){
		 ...
		 rcu_read_unlock();
		if(n_fibers==0){
					regs->cx=(unsigned long) n_fibers;
...
int Pre_Handler_Exit(struct kprobe *p, struct pt_regs *regs){ 
	...
	int i;
	int j;
	printk(KERN_INFO "DEBUG EXIT HANDLER\n");
	...
	hash_for_each_possible_rcu(processi, fiber_processo, node,current->tgid){
		...
		hash_for_each_safe(fiber_processo->listathread, j, thread_node, lista_thread_iter, node){

			if (lista_thread_iter->id == current->pid) {
				hash_for_each_safe(fiber_processo->listafiber, i, fib_node, lista_fiber_iter, node){
					if(lista_fiber_iter->regs!=NULL){
						kfree(lista_fiber_iter->regs);
					}
					if(lista_fiber_iter->fpu!=NULL){
				
						kfree(lista_fiber_iter->fpu);
					}
					if(lista_fiber_iter->bitmap_fls!=NULL){
						kfree(lista_fiber_iter->bitmap_fls);
					}
				
					if(lista_fiber_iter->fls!=NULL){
						kfree(lista_fiber_iter->fls);
					}
					hash_del_rcu(&(lista_fiber_iter->node));
					if(lista_fiber_iter!=NULL){

						kfree(lista_fiber_iter);
					}
				
				}
				
				hash_del_rcu(&(lista_thread_iter->node));
		
				if(lista_thread_iter!=NULL){
					kfree(lista_thread_iter);
				}
        ...
        if (fiber_processo->id == pid) {
					...
					hash_for_each_possible_rcu(fiber_processo->listafiber, lista_fiber_iter, node, fib_id){
							if (lista_fiber_iter->id == fib_id) {
								...
								if (lista_fiber_iter->selected_thread==NULL){
									running_str = "not ";
								} else {
									running_str = "";
...
static struct Fiber* do_fib_create(void* func,void* parameters, void *stack_pointer, unsigned long stack_size,struct Process* str_processo,int flag){
	...
	sprintf (fibers_entry_log.name, "f%d", str_fiber->id);
